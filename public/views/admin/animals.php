    ...
    <div class="card">
        ...
        <form method="post" enctype="multipart/form-data" class="admin-animal-form">
            ...
            <label>Bild aus Medienbibliothek
                <select name="media_asset_id" data-media-select>
                    ...
                    <?php foreach ($mediaAssets as $asset): ?>
                        <option value="<?= (int)$asset['id'] ?>" <?= (($editAnimal['media_asset_id'] ?? null) == $asset['id']) ? 'selected' : '' ?> data-media-preview="<?= htmlspecialchars(media_url($asset['file_path'] ?? null) ?? '') ?>">
            ...
            <div class="media-picker-preview" data-media-preview>
                <?php if ($src = media_url($editAnimal['image_path'] ?? null)): ?>
                    <img src="<?= htmlspecialchars($src) ?>" alt="Aktuelles Tierbild" class="mb-2 h-40 w-full rounded-2xl object-cover">
                <?php else: ?>
            ...
            <input type="hidden" name="image_path" value="<?= htmlspecialchars(normalize_media_path($editAnimal['image_path'] ?? '')) ?>">
            <label>Besitzer
